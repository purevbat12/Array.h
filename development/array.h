		...
		if(arr->dimension == 1){
			...
			while(true){
				...
				while(temp2->dimension >= 1){
					printf("temp2->dimension = %llu\n", temp2->dimension);
					if(temp2->dimension == 1){
						printf("temp2->dimension == 1\n");
						index--;
						printArr(recordOfIndices, "Before the change to recordOfIndices.\n\n");
						bool differenceIsOne = false, hasGoneThroughFirstIfCondition = false;
						if(temporaryRecord->size > 1){
							size_t count = 0;
							for(size_t a = 0; a < temporaryRecord->size - 1; a++){
								if(abs(*(int*)at(temporaryRecord, a) - *(int*)at(temporaryRecord, a + 1)) == 1){
									count++;
								}
							}
							if(count == temporaryRecord->size - 1){
								differenceIsOne = true;
							}
						}
						if(temp3->size - 1 == *(int*)at(recordOfIndices, index)){
							...
							hasGoneThroughFirstIfCondition = true;
							printf("has gone through the first if conditional so it is true!\n\n");
						}
						if(temporaryRecord->size != 0 && (temporaryRecord->size > 1 && differenceIsOne)){
							printf("in the second conditional.\n");
							for(size_t a = (size_t)*((int*)at(temporaryRecord, 0)); a < temporaryRecord->size; a++){
								assign(recordOfIndices, 1, i(0), a);
							}
							assign(recordOfIndices, 1, i(*(int*)at(recordOfIndices, (size_t)*(int*)at(temporaryRecord, 0)) + 1), (size_t)*(int*)at(temporaryRecord, 0) - 1);
							
						}
						else if(!hasGoneThroughFirstIfCondition){
							printf("in the other else if conditional with %s.\n", hasGoneThroughFirstIfCondition ? "true" : "false");
						...
						printArr(flatArrays, "Testing out the flat arrays.");
						printf("\n\n");
						printArr(recordOfIndices, "Changed state.\n\n");
						index = 0;
						break;
					}
					else{
						...
						printArr(temp3, "Testing temp3");
						printArr(temp2, "Before temp2");
						temp2 = *((array**)at(temp2, *(int*)at(recordOfIndices, index)));
						printArr(temp2, "After temp2");
						printf("index = %d\t recordOfIndices[%d] = %d\ttemp3->size = %llu\n", index, index, *(int*)at(recordOfIndices, index), temp3->size);
						printf("index != recordOfIndices->size - 1 <-> %d != %d <-> %d\trecordOfIndices[%d] == temp3->size - 1 <-> %d == %d <-> %d\n", index, recordOfIndices->size - 1, index != recordOfIndices->size - 1, index, *(int*)at(recordOfIndices, index), (int)temp3->size - 1, *(int*)at(recordOfIndices, index) == (int)temp3->size - 1);
						if((index != recordOfIndices->size - 1) && (*(int*)at(recordOfIndices, index) == temp3->size - 1)){
							printf("%d == %llu\n\n\n\n\n\n\n\n\n\n\n\n\n\n", *(int*)at(recordOfIndices, index), temp3->size);
							insertEnd(temporaryRecord, 1, i(index - 1));
							printArr(temporaryRecord, "");
						}
						printArr(temporaryRecord, "");
						index++;
					}
				...
				size_t b = 0;
				printArr(indicesCheck, "Checking indicesCheck");
				for(size_t a = 0; a < indicesCheck->size; a++){
					if(*((bool*)at(indicesCheck, a))){
						printf("bool checked true.\n\n");
						b++;
					}
				}
				if(b == indicesCheck->size){
					...
					printf("Not all checked to be true. b = %llu\n", b);
				}
				printf("\nNext while(true){} loop.\n\n");
			}
			return flatArrays;
		}
	}

...
#define createArr(name, form, count, data) \
	...
	name->format = form; \
	name->size = count; \
	name->label = malloc(sizeof(#name)); \
	strcpy(name->label, #name); \
	name->arr = malloc(count * sizeOfElement(form)); \
	if(name->arr == NULL){ \
		printf("Memory allocation failed! Exit code 1.\nExiting..."); \
		exit(1); \
	} \
	memcpy(name->arr, data, count * sizeOfElement(form)); 
...
void *at(array *arr, size_t index){
	...
	void *elem = (char*)(arr->arr) + elemSize * index;
	return elem;
}
void printArr(array *arr, char *desc){
	if(arr->size == 0){
